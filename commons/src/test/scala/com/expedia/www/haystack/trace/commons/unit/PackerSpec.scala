    ...
    it("should pack and unpack spanBuffer proto object for all packer types") {
      PackerType.values.foreach(packerType => {
        ...
        val span = Span.newBuilder()
          .setTraceId("trace-1")
          .setSpanId("span-id")
          .setServiceName("myservice").setOperationName("myoperation")
          .addTags(Tag.newBuilder().setKey("error").setVBool(false).build())
          .build()
        val spanBuffer = SpanBuffer.newBuilder().setTraceId("trace-1").addChildSpans(span).addChildSpans(span).build()
