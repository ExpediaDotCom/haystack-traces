...
import com.expedia.www.haystack.span.bufferer.integration.serdes.{SpanBufferProtoDeserializer, SpanProtoSerializer}
...
abstract class BaseIntegrationTestSpec extends WordSpec with GivenWhenThen with Matchers with BeforeAndAfterAll with BeforeAndAfterEach {
  ...
  protected val SPAN_BUFFERING_WINDOW_MS = 6000L
  protected val AUTO_COMMIT_INTERVAL_MS = 3000
  protected val INITIAL_STORE_CAPACITY = 100
  protected val MAX_SPAN_BUFFERS_IN_MEM = 500
  ...
  protected val OUTPUT_TOPIC = "spanbuffer"
  protected val CHANGELOG_TOPIC = s"$APP_ID-${StreamTopology.STATE_STORE_NAME}-changelog"
  ...
  override def beforeEach() {
    ...
    PRODUCER_CONFIG.put(ProducerConfig.VALUE_SERIALIZER_CLASS_CONFIG, classOf[SpanProtoSerializer])
      ...
      override def run(): Unit = {
        if(idx < maxSpans) {
          ...
          IntegrationTestUtils.produceKeyValuesSynchronouslyWithTimestamp(
            ...
            PRODUCER_CONFIG,
            timestamp)
          timestamp = timestamp + (SPAN_BUFFERING_WINDOW_MS / (maxSpans - 1))
        }
        idx = idx + 1
      }
  ...
  protected def validateChildSpans(spanBuffer: SpanBuffer,
    ...
    (0 until spanBuffer.getChildSpansCount).toList foreach { idx =>
      ...
      spanBuffer.getChildSpans(idx).getOperationName shouldBe "some-op"
    }
  }
}
