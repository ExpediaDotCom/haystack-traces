...
package com.expedia.www.haystack.span.collector.writers.cassandra
...
import com.expedia.www.haystack.span.collector.config.entities.CassandraConfiguration
import com.expedia.www.haystack.span.collector.metrics.AppMetricNames
import com.expedia.www.haystack.span.collector.writers.cassandra.Schema._
...
class CassandraWriter(config: CassandraConfiguration)(implicit val dispatcher: ExecutionContextExecutor) extends SpanBufferWriter {
  ...
  /**
    * writes the span records to cassandra. Use the current timestamp as the sort key for the writes to same
    * TraceId
    * @param records list of span buffer records. Each buffer contains the list of child spans belonging to a trace
    * @return
    */
  override def write(records: Seq[SpanBufferRecord]): Future[_] = {
    try {
      val futures: Seq[Future[Boolean]] = records.map { rec =>
        val timer = writeTimer.time()
        val promise = Promise[Boolean]()
        val batchStatement = prepareBatchStatement(rec.spanBuffer.getTraceId, rec.spanBufferProtoBytes)
  ...
  private def prepareBatchStatement(traceId: String, spansByte: Array[Byte]): Statement = {
    new BoundStatement(insertSpan)
      ...
      .setBytes(SPANS_COLUMN_NAME, ByteBuffer.wrap(spansByte))
