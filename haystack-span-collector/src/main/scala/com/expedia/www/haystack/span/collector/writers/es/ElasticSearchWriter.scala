...
package com.expedia.www.haystack.span.collector.writers.es

import java.text.SimpleDateFormat
import java.util.Date

import com.expedia.open.tracing.buffer.SpanBuffer
import com.expedia.www.haystack.span.collector.config.entities.{ElasticSearchConfiguration, IndexConfiguration}
import com.expedia.www.haystack.span.collector.metrics.AppMetricNames
import com.expedia.www.haystack.span.collector.writers.es.index.document.IndexDocumentGenerator
import com.expedia.www.haystack.span.collector.writers.{SpanBufferRecord, SpanBufferWriter}
...
class ElasticSearchWriter(esConfig: ElasticSearchConfiguration, indexConf: IndexConfiguration) extends SpanBufferWriter {
  ...
  /**
    * writes the span buffer records to elastic search asynchronously
    * @param elems list of span buffer records
    * @return Future that contains the success or failure of the write operation
    */
  override def write(elems: Seq[SpanBufferRecord]): Future[_] = {
    try {
  ...
  /**
    * Builds the bulk index request for the list of span buffers.
    * Each span buffer record is converted to an index document for elastic search write
    * @param records a set of span buffer records
    * @return
    */
  private def buildBulkIndexingRequest(records: Seq[SpanBufferRecord]): Bulk = {
    val bulkActions = new Bulk.Builder()

    for(rec <- records;
        action = createIndexAction(rec.spanBuffer, indexName()); if action.isDefined) {
  ...
  private def createIndexAction(spanBuffer: SpanBuffer, indexName: String): Option[BulkableAction[DocumentResult]] = {
    // add all the spans as one document
    val idxDocument = documentGenerator.createIndexDocument(spanBuffer)
    ...
    idxDocument match {
      case Some(doc) =>
        Some(new Index.Builder(doc.json)
          ...
          .build())
      case _ =>
        LOGGER.info("Fail to convert the span buffer record to an index document!")
        None
    }
  }
