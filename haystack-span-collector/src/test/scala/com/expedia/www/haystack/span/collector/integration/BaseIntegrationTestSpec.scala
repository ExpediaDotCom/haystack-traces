...
abstract class BaseIntegrationTestSpec extends WordSpec with GivenWhenThen with Matchers with BeforeAndAfterAll with BeforeAndAfterEach {
  ...
  protected val CONSUMER_TOPIC = "span-buffer"
  ...
  protected def produceToKafka(spanBuffers: Seq[SpanBuffer]): Unit = {
    spanBuffers foreach { st =>
      ...
      producer.send(record, new Callback() {
        override def onCompletion(metadata: RecordMetadata, exception: Exception): Unit = {
          if (exception != null) {
            fail("Fail to produce the span buffers to kafka with error", exception)
          }
        }
      })
    }
  ...
  protected def queryAllCassandra(): Seq[CassandraRow] = {
    val rows = cassandraSession.execute("SELECT id, ts, spans from traces")
    rows.map(row => {
      CassandraRow(row.getString("id"), row.getTimestamp("ts"), SpanBuffer.parseFrom(row.getBytes("spans")))
    }).toList
  }
  ...
  protected def createSpanBuffers(total: Int, spanCount: Int, duration: Long): Seq[SpanBuffer] = {
    (0 until total).toList map { traceId =>
      val spanBufferBuilder = SpanBuffer.newBuilder()
      spanBufferBuilder.setTraceId(traceId.toString)
      ...
      (0 until spanCount).toList foreach { spanId =>
        ...
        val span = Span.newBuilder()
          ...
          .build()
        spanBufferBuilder.addChildSpans(span)
      }
      spanBufferBuilder.build()
    }
  }
