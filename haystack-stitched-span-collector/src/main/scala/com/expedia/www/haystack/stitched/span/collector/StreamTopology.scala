...
package com.expedia.www.haystack.stitched.span.collector
...
import com.expedia.www.haystack.stitched.span.collector.config.entities.CollectorConfiguration
...
class StreamTopology(collectorConfig: CollectorConfiguration,
  ...
  /**
    ...
    * The topology reads stitched spans from kafka, groups them, writes to cassandra and elastic search together
  ...
  def start(): (KillSwitch, Future[Done]) = {
    ...
    Consumer
      ...
      .mapAsync(collectorConfig.parallelism) { writeStitchedSpans }
  ...
  /**
    * writes the stitched span records to elastic search and cassandra asynchronously
    * @param records set of stitched span records
    ...
    */
  private def writeStitchedSpans(records: Seq[CommittableMessage[Array[Byte], Array[Byte]]]): Future[Seq[CommittableOffset]] = {
    val promise = Promise[Seq[CommittableMessage[Array[Byte], Array[Byte]]]]()

    val stitchedSpans: Seq[StitchedSpanDataRecord] = records
      .map(rec => StitchedSpanDataRecord(stitchedSpanDeser.deserialize(rec.record.value()), rec.record.value()))
