...
object SpanBufferStoreChangeLogger extends MetricsSupport {
  protected val LOGGER: Logger = LoggerFactory.getLogger(SpanBufferStoreChangeLogger.getClass)
...
/**
  ...
  */
class SpanBufferStoreChangeLogger[K, V](val name: String,
  ...
  def logChange(key: K, value: V): Unit = {
    if (collector != null) {
      ...
      try {
        ...
        case ex: Exception =>
          SpanBufferStoreChangeLogger.LOGGER.error(s"Fail to add the change in the changelog topic=$topic, " +
            s"partition=$partition", ex)
          SpanBufferStoreChangeLogger.changeLogFailure.mark()
      }
    }
  }
}
