...
class ConfigurationReloadTraceBackendProvider(reloadConfig: AutoDiscoveryReloadConfiguration)
  ...
  override def load(): Unit = {
    reloadConfig.observers.foreach(observer => {
      ...
      RetryOperation.executeWithRetryBackoff(() => AwsElbDiscoverer.discoverElbs(reloadConfig.region,reloadConfig.tags), RetryOperation.Config(3, 1000, 2)) match {
        case Success(result) =>
          if (result.nonEmpty) {
