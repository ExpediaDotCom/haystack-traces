...
object OrphanedTraceTransformerConstants {
  val UNKNOWN_OPERATION_NAME = "unknown operation"
  val UNKNOWN_SERVICE_NAME = "unknown service"
}
...
class OrphanedTraceTransformer extends TraceTransformer {
  ...
  def generateRootSpan(orphanedParents: mutable.HashMap[String, ArrayBuffer[Span]]): Span = {
    ...
    orphanedParents.values.head.foreach(span => {
      if (span.getStartTime < startTime) {
        startTime = span.getStartTime
      }
      val spanEndTime = span.getDuration + span.getStartTime
      if (endTime < spanEndTime) {
        endTime = spanEndTime
      }
    })
    Span.newBuilder().setDuration(endTime - startTime).setOperationName(OrphanedTraceTransformerConstants.UNKNOWN_OPERATION_NAME).setServiceName(OrphanedTraceTransformerConstants.UNKNOWN_SERVICE_NAME)
      .setSpanId(span.getTraceId).setStartTime(startTime).setTraceId(span.getTraceId).build()
  }
}
