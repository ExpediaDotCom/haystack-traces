...
object SpanMerger {
  ...
  def mergeParentChildSpans(parentSpan: Span, childSpan: Span): Span = _merge(clientSpan = parentSpan, serverSpan = childSpan)
  ...
  def mergeSpans(spans: Seq[Span]): Span = {
    ...
    (clientSpanOptional, serverSpanOptional) match {
      ...
      case (Some(clientSpan), Some(serverSpan)) => _merge(clientSpan, serverSpan)
  ...
  private def _merge(clientSpan: Span, serverSpan: Span): Span = {
    Span
