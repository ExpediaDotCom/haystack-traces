...
import com.expedia.www.haystack.span.stitcher.serde.SpanSerde
...
class SpanSerdeSpec extends FunSpec with Matchers {
  ...
  describe("Span Serde") {
    it("should serialize and deserialize a span object") {
      val ser = SpanSerde.serializer.serialize(TOPIC, testSpan)
      val deser = SpanSerde.deserializer.deserialize(TOPIC, ser)
    ...
    it("should return null on serializing invalid  span bytes") {
      ...
      val deser = SpanSerde.deserializer.deserialize("topic", data)
      deser shouldBe null
    }
  }
}
